...
This scraper extracts job listings from Omniconvert careers page (Romania only) and imports them to peviitor.ro.

Target: https://www.omniconvert.com/en/jobs/romania
...
1. **Start with brand** - We know the brand (e.g., "Omniconvert")
...
8. **Scrape new jobs** - Extract jobs from Omniconvert careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Omniconvert Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Omniconvert careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Omniconvert API (jobs for Romania)
    │
    ▼
